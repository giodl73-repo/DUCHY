...
| 319 concrete source records have passed the gate, but most source-backed titles still have no reviewed parentage. | Historical identity coverage is broader than hierarchy coverage. | Expand parentage through reviewed relation packets only. |
| 500 CK3 Europe county candidates are staged as pending discovery rows only. | The repo now has a larger search queue, not 500 additional accepted source-backed facts. | Resolve each row to independent historical sources before source stubs or promotion. |
...
| Only 140 real parentage fixtures have passed review. | Source-backed title paths remain sparse relative to the 319-source title catalog. | Expand parentage only through reviewed source packets. |
