...
| Only two concrete source records have passed the gate. | Historical import is real but intentionally tiny. | Expand through reviewed source packets only. |
...
| No real parentage fixture has passed review yet. | Source-backed title paths remain one-step for real imported titles. | Add parentage only through reviewed source packets. |
