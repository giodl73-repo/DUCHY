...
fn run() -> Result<(), Vec<String>> {
    ...
    if let [command, manifest_path, output_path] = args.as_slice() {
        ...
        if command == "parentage-rank-skip-bridge-cluster-report" {
            return parentage_rank_skip_bridge_cluster_report(manifest_path, output_path);
        }
    ...
    if let [command, sources_path, facts_path, output_path] = args.as_slice() {
        ...
        if command == "parentage-rank-skip-relation-report" {
            return parentage_rank_skip_relation_report(sources_path, facts_path, output_path);
        }
        if command == "parentage-rank-skip-bridges-tsv" {
            return parentage_rank_skip_bridges_tsv(sources_path, facts_path, output_path);
        }
    ...
    if let [command, manifest_path, output_dir, chunk_size] = args.as_slice() {
        ...
        if command == "parentage-rank-skip-bridge-cluster-review-shard" {
            return parentage_rank_skip_bridge_cluster_review_shard(
                manifest_path,
                output_dir,
                chunk_size,
            );
    ...
    let (sources_path, facts_path) = match args.as_slice() {
        ...
        _ => {
            return Err(vec![
                "usage: duchy-import [status [sources-file facts-file]] | manifest manifest-file | source-stubs manifest-file output.sources | rejected-report manifest-file output.md | active-manifest manifest-file output.manifest | archive-manifest manifest-file output.manifest | manifest-report manifest-file output.md | duplicate-url-report manifest-file output.md | manifest-tsv manifest-file output.tsv | manifest-from-tsv input.tsv output.manifest | shard-manifest manifest-file output-dir chunk-size | parentage-coverage-report sources-file facts-file output.md | parentage-change-report sources-file facts-file output.md | parentage-graph-report sources-file facts-file output.md | parentage-rank-skip-tsv sources-file facts-file output.tsv | parentage-rank-skip-candidates sources-file facts-file output.md | parentage-rank-skip-relation-report sources-file facts-file output.md | parentage-rank-skip-bridges-tsv sources-file facts-file output.tsv | parentage-rank-skip-shard input.tsv output-dir chunk-size | parentage-rank-skip-report input.tsv output.md | parentage-rank-skip-bridge-shard input.tsv output-dir chunk-size | parentage-rank-skip-bridge-report input.tsv output.md | parentage-rank-skip-bridge-clusters-tsv input.tsv output.tsv | parentage-rank-skip-bridge-cluster-report input.tsv output.md | parentage-rank-skip-bridge-cluster-review-tsv input.tsv output.tsv | parentage-rank-skip-bridge-cluster-review-report input.tsv output.md | parentage-rank-skip-bridge-cluster-packet-stubs input.tsv output.md | parentage-rank-skip-bridge-cluster-review-shard input.tsv output-dir chunk-size | parentage-gap-tsv sources-file facts-file output.tsv [blockers.tsv] | parentage-gap-shard input.tsv output-dir chunk-size | parentage-gap-report input.tsv output.md".to_string(),
            ])
        }
    };
...
fn parentage_rank_skip_relation_report(
    sources_path: &str,
    facts_path: &str,
    output_path: &str,
) -> Result<(), Vec<String>> {
    ...
    let relations = duchy::source_backed_relations_from_facts(&catalog, &facts, &timeline)?;
    ...
    let parentage_edges = active_parentage_edges(&facts);
    let rank_skip_rows = parentage_rank_skip_rows(&parentage_edges, &title_by_id);
    let explanations = parentage_rank_skip_relation_explanations(&rank_skip_rows, &relations);

    let explained_count = explanations
        .iter()
        .filter(|explanation| !explanation.relations.is_empty())
        .count();
    let mut by_relation_kind: BTreeMap<&str, usize> = BTreeMap::new();
    for explanation in &explanations {
        let mut kinds_for_row = BTreeSet::new();
        for relation in &explanation.relations {
            kinds_for_row.insert(relation.relation_kind.as_str());
        }
        for relation_kind in kinds_for_row {
            *by_relation_kind.entry(relation_kind).or_default() += 1;
        }
    }

    let mut output = String::new();
    output.push_str("# DUCHY Parentage Rank Skip Relation Report\n\n");
    output.push_str(&format!("sources: {}\n", catalog.record_count()));
    output.push_str(&format!("facts: {}\n", facts.len()));
    output.push_str(&format!("titles: {}\n", titles.len()));
    output.push_str(&format!("relation_facts: {}\n", relations.len()));
    output.push_str(&format!("rank_skip_rows: {}\n", rank_skip_rows.len()));
    output.push_str(&format!("relation_explained_rows: {explained_count}\n"));
    output.push_str(&format!(
        "unexplained_rows: {}\n\n",
        rank_skip_rows.len().saturating_sub(explained_count)
    ));
    ...
    output.push_str("| Relation Kind | Rows |\n");
    output.push_str("|---|---:|\n");
    for (relation_kind, count) in &by_relation_kind {
        output.push_str(&format!("| {relation_kind} | {count} |\n"));
    }
    if by_relation_kind.is_empty() {
    ...
    for explanation in &explanations {
        ...
        output.push_str(&format!("- span: {}\n", row.span));
        if explanation.relations.is_empty() {
            output.push_str("- relation_context: none\n\n");
            continue;
        }
    ...
    println!("DUCHY parentage rank skip relation report");
    println!("- titles: {}", titles.len());
    println!("- relation facts: {}", relations.len());
    println!("- rank skip rows: {}", rank_skip_rows.len());
    println!("- relation explained rows: {explained_count}");
    println!("- output: {output_path}");

    Ok(())
}

fn parentage_rank_skip_bridges_tsv(
    sources_path: &str,
    facts_path: &str,
    output_path: &str,
) -> Result<(), Vec<String>> {
    ...
    for row in &rank_skip_rows {
        ...
        bridge_rows.push(parentage_rank_skip_bridge_tsv_row(row, &candidate));
    }
    ...
    println!("DUCHY parentage rank skip bridges TSV");
    println!("- titles: {}", titles.len());
    println!("- rank skip rows: {}", rank_skip_rows.len());
    println!("- bridge rows: {}", bridge_rows.len());
    println!("- output: {output_path}");
...
fn parentage_rank_skip_bridge_cluster_review_shard(
    input_path: &str,
    output_dir: &str,
    chunk_size: &str,
) -> Result<(), Vec<String>> {
    ...
    let mut index = String::new();
    index.push_str("# DUCHY Parentage Rank Skip Bridge Cluster Review Shards\n\n");
    index.push_str(&format!("source_tsv: {input_path}\n"));
    index.push_str(&format!("review_rows: {}\n", rows.len()));
    index.push_str(&format!("chunk_size: {chunk_size}\n\n"));
    index.push_str("| Shard | Rows | Pending | Not Inferred | High | Medium | Low |\n");
    index.push_str("|---|---:|---:|---:|---:|---:|---:|\n");

    for (index_number, chunk) in rows.chunks(chunk_size).enumerate() {
        let shard_name = format!("batch-{:03}.tsv", index_number + 1);
        let shard_path = Path::new(output_dir).join(&shard_name);
        let shard_text = parentage_rank_skip_bridge_cluster_review_rows_to_tsv(chunk);
        fs::write(&shard_path, shard_text)
            .map_err(|error| vec![format!("failed to write {}: {error}", shard_path.display())])?;

        let counts = parentage_rank_skip_bridge_cluster_review_counts(chunk);
        index.push_str(&format!(
            "| {shard_name} | {} | {} | {} | {} | {} | {} |\n",
            chunk.len(),
            counts.pending,
    ...
    println!("DUCHY parentage rank skip bridge cluster review shards");
...
fn parentage_rank_skip_bridge_cluster_report(
    input_path: &str,
    output_path: &str,
) -> Result<(), Vec<String>> {
    let input_text = fs::read_to_string(input_path)
        .map_err(|error| vec![format!("failed to read {input_path}: {error}")])?;
    let rows = parentage_rank_skip_bridge_cluster_rows_from_tsv(&input_text)?;
    if rows.is_empty() {
        return Err(vec![format!(
            "{input_path} has no parentage rank skip bridge cluster rows"
        )]);
    }

    let child_total = rows.iter().map(|row| row.child_count).sum::<usize>();
    let high_total = rows.iter().map(|row| row.high).sum::<usize>();
    let medium_total = rows.iter().map(|row| row.medium).sum::<usize>();
    let low_total = rows.iter().map(|row| row.low).sum::<usize>();

    let mut output = String::new();
    output.push_str("# DUCHY Parentage Rank Skip Bridge Cluster Report\n\n");
    output.push_str(&format!("source_tsv: {input_path}\n"));
    output.push_str(&format!("clusters: {}\n", rows.len()));
    output.push_str(&format!("clustered_children: {child_total}\n"));
    output.push_str(&format!("high_priority_rows: {high_total}\n"));
    output.push_str(&format!("medium_priority_rows: {medium_total}\n"));
    output.push_str(&format!("low_priority_rows: {low_total}\n\n"));

    output.push_str("## Interpretation\n\n");
    output.push_str("- Clusters are packet-planning leads, not import-ready parentage claims.\n");
    output.push_str("- A large cluster usually means many skipped edges share one accepted same-parent bridge candidate.\n");
    output.push_str("- Reviewers must verify child-to-candidate parentage from sources before replacing direct skipped parentage.\n\n");

    output.push_str("## Cluster Summary\n\n");
    output.push_str("| Candidate Parent | Current Parent | Expected Rank | Children | High | Medium | Low | Span Range |\n");
    output.push_str("|---|---|---|---:|---:|---:|---:|---|\n");
    for row in &rows {
        output.push_str(&format!(
            "| {} | {} | {} | {} | {} | {} | {} | {} |\n",
            markdown_escape(&row.candidate_parent_name),
            markdown_escape(&row.current_parent_name),
            markdown_escape(&row.expected_parent_rank),
            row.child_count,
            row.high,
            row.medium,
            row.low,
            markdown_escape(&row.span_range)
        ));
    }
    ...
    println!("DUCHY parentage rank skip bridge cluster report");
    println!("- clusters: {}", rows.len());
    println!("- clustered children: {child_total}");
    println!("- output: {output_path}");

    Ok(())
}

fn parentage_rank_skip_bridge_cluster_review_tsv(
    input_path: &str,
    output_path: &str,
) -> Result<(), Vec<String>> {
    let input_text = fs::read_to_string(input_path)
...
#[derive(Clone)]
struct ParentageRankSkipBridgeClusterTsvRow {
    candidate_parent_id: String,
    candidate_parent_name: String,
    current_parent_id: String,
    current_parent_name: String,
    ...
    child_ids: String,
    child_names: String,
    skip_fact_ids: String,
    bridge_fact_ids: String,
}

#[derive(Clone)]
struct ParentageRankSkipBridgeClusterReviewTsvRow {
...
#[derive(Default)]
struct ParentageRankSkipBridgeClusterReviewCounts {
    pending: usize,
    not_inferred: usize,
    high: usize,
    medium: usize,
...
#[derive(Debug)]
struct ParentageRankSkipRelationExplanation<'a> {
    row: &'a ParentageRankSkipRow,
    relations: Vec<ParentageRankSkipRelationMatch>,
}

#[derive(Debug, Clone)]
struct ParentageRankSkipRelationMatch {
    relation_kind: duchy::RelationKind,
    span: duchy::YearSpan,
    overlap: String,
}
...
fn parentage_rank_skip_relation_explanations<'a>(
    rows: &'a [ParentageRankSkipRow],
    relations: &[duchy::RelationSpan],
) -> Vec<ParentageRankSkipRelationExplanation<'a>> {
    let mut explanations = Vec::new();
    for row in rows {
        let Some(skip_span) = year_span_from_label(&row.span) else {
            explanations.push(ParentageRankSkipRelationExplanation {
                row,
                relations: Vec::new(),
            });
            continue;
        };
        let mut relation_matches = relations
            .iter()
            .filter(|relation| {
                relation.subject_title_id == row.child_id
                    && relation.related_title_id == row.parent_id
            })
            .filter_map(|relation| {
                span_overlap_label(&relation.span, &skip_span).map(|overlap| {
                    ParentageRankSkipRelationMatch {
            ...
            .collect::<Vec<_>>();
        relation_matches.sort_by(|left, right| {
            left.relation_kind
...
fn parentage_rank_skip_bridge_cluster_review_tsv_header() -> &'static str {
...
fn parentage_rank_skip_bridge_cluster_rows_from_tsv(
    input: &str,
) -> Result<Vec<ParentageRankSkipBridgeClusterTsvRow>, Vec<String>> {
    let mut lines = input.lines();
    let Some(header) = lines.next() else {
        return Err(vec![
            "parentage rank skip bridge cluster TSV is empty".to_string()
        ]);
    };
    let expected_header = parentage_rank_skip_bridge_cluster_tsv_header();
    if header != expected_header {
        return Err(vec![format!(
            "invalid parentage rank skip bridge cluster TSV header: expected {expected_header}"
        )]);
    }
    ...
    for (line_index, line) in lines.enumerate() {
        ...
        rows.push(ParentageRankSkipBridgeClusterTsvRow {
            candidate_parent_id: fields[0].to_string(),
            candidate_parent_name: fields[1].to_string(),
            current_parent_id: fields[2].to_string(),
            current_parent_name: fields[3].to_string(),
            expected_parent_rank: fields[4].to_string(),
            child_count: fields[5].parse::<usize>().map_err(|error| {
                vec![format!("line {line_number}: invalid child_count: {error}")]
            })?,
            high: fields[6]
                .parse::<usize>()
                .map_err(|error| vec![format!("line {line_number}: invalid high: {error}")])?,
            medium: fields[7]
                .parse::<usize>()
                .map_err(|error| vec![format!("line {line_number}: invalid medium: {error}")])?,
            low: fields[8]
                .parse::<usize>()
                .map_err(|error| vec![format!("line {line_number}: invalid low: {error}")])?,
            span_range: fields[9].to_string(),
            child_ids: fields[10].to_string(),
            child_names: fields[11].to_string(),
            skip_fact_ids: fields[12].to_string(),
            bridge_fact_ids: fields[13].to_string(),
        });
    }
    rows.sort_by(|left, right| {
        right
...
fn parentage_rank_skip_bridge_cluster_review_rows_from_tsv(
    input: &str,
) -> Result<Vec<ParentageRankSkipBridgeClusterReviewTsvRow>, Vec<String>> {
    let mut lines = input.lines();
    let Some(header) = lines.next() else {
        return Err(vec![
            "parentage rank skip bridge cluster review TSV is empty".to_string(),
        ]);
    };
    let expected_header = parentage_rank_skip_bridge_cluster_review_tsv_header();
    if header != expected_header {
        return Err(vec![format!(
            "invalid parentage rank skip bridge cluster review TSV header: expected {expected_header}"
        )]);
    }
    ...
    for (line_index, line) in lines.enumerate() {
        ...
        if fields.len() != 18 {
            return Err(vec![format!(
                "line {line_number}: expected 18 TSV columns, found {}",
                fields.len()
            )]);
        }
        rows.push(ParentageRankSkipBridgeClusterReviewTsvRow {
            review_status: fields[0].to_string(),
            review_disposition: fields[1].to_string(),
            candidate_parent_id: fields[2].to_string(),
            candidate_parent_name: fields[3].to_string(),
            current_parent_id: fields[4].to_string(),
            current_parent_name: fields[5].to_string(),
            expected_parent_rank: fields[6].to_string(),
            child_count: fields[7].parse::<usize>().map_err(|error| {
                vec![format!("line {line_number}: invalid child_count: {error}")]
            })?,
            high: fields[8]
                .parse::<usize>()
            ...
            low: fields[10]
                .parse::<usize>()
                .map_err(|error| vec![format!("line {line_number}: invalid low: {error}")])?,
            span_range: fields[11].to_string(),
            evidence_needed: fields[12].to_string(),
            review_note: fields[13].to_string(),
            child_ids: fields[14].to_string(),
            child_names: fields[15].to_string(),
            skip_fact_ids: fields[16].to_string(),
            bridge_fact_ids: fields[17].to_string(),
        });
    }
    Ok(rows)
}

fn parentage_rank_skip_bridge_cluster_review_rows_to_tsv(
    rows: &[ParentageRankSkipBridgeClusterReviewTsvRow],
) -> String {
    let mut output = String::new();
    output.push_str(parentage_rank_skip_bridge_cluster_review_tsv_header());
    output.push('\n');
    for row in rows {
        output.push_str(&parentage_rank_skip_bridge_cluster_review_row_to_tsv(row));
        output.push('\n');
    }
    output
}
...
fn parentage_rank_skip_bridge_cluster_review_counts(
    rows: &[ParentageRankSkipBridgeClusterReviewTsvRow],
) -> ParentageRankSkipBridgeClusterReviewCounts {
    let mut counts = ParentageRankSkipBridgeClusterReviewCounts::default();
    for row in rows {
        if row.review_status == "pending_review" {
            counts.pending += 1;
        }
        if row.review_disposition == "not_inferred" {
            counts.not_inferred += 1;
        }
        counts.high += row.high;
        counts.medium += row.medium;
        counts.low += row.low;
    }
    counts
}
...
fn parentage_rank_skip_bridge_tsv_row(
    row: &ParentageRankSkipRow,
    candidate: &ParentageRankSkipCandidate,
) -> String {
    [
        tsv_escape(&row.fact_id),
        tsv_escape(&row.child_id),
        tsv_escape(&row.child_name),
        title_rank_label(row.child_rank).to_string(),
        title_rank_label(row.expected_parent_rank).to_string(),
        tsv_escape(&row.parent_id),
        tsv_escape(&row.parent_name),
        title_rank_label(row.parent_rank).to_string(),
        tsv_escape(&row.span),
        tsv_escape(&candidate.title_id),
        tsv_escape(&candidate.title_name),
        tsv_escape(&candidate.exists),
        candidate.overlap_years.to_string(),
        tsv_escape(candidate.bridge_fact_id.as_deref().unwrap_or("")),
        parentage_rank_skip_priority(row.child_rank, row.parent_rank).to_string(),
        tsv_escape(parentage_rank_skip_note(row.child_rank, row.parent_rank)),
    ]
    .join("\t")
}
...
mod tests {
    ...
    #[test]
    fn explains_rank_skip_rows_with_overlapping_relation_to_current_parent() {
        let row = ParentageRankSkipRow {
            fact_id: "fact-demo-skip".to_string(),
            child_id: "title-demo-child".to_string(),
            child_name: "Demo Child".to_string(),
            ...
            parent: "Demo Empire (title-demo-empire)".to_string(),
            parent_rank: duchy::TitleRank::Empire,
            span: "1000..1100".to_string(),
        };
        let relations = vec![
            duchy::RelationSpan {
                subject_title_id: "title-demo-child".to_string(),
                related_title_id: "title-demo-empire".to_string(),
                relation_kind: duchy::RelationKind::ImperialState,
                span: duchy::YearSpan::new(1050, Some(1150)),
            },
            duchy::RelationSpan {
                subject_title_id: "title-demo-child".to_string(),
                related_title_id: "title-other-empire".to_string(),
                relation_kind: duchy::RelationKind::VassalageOrSuzerainty,
                span: duchy::YearSpan::new(1000, Some(1100)),
            },
            duchy::RelationSpan {
                subject_title_id: "title-demo-child".to_string(),
                related_title_id: "title-demo-empire".to_string(),
                relation_kind: duchy::RelationKind::SplitFiefOrControl,
                span: duchy::YearSpan::new(1200, Some(1250)),
            },
        ];

        let rows = vec![row];
        let explanations = parentage_rank_skip_relation_explanations(&rows, &relations);

        assert_eq!(explanations.len(), 1);
        assert_eq!(explanations[0].relations.len(), 1);
        assert_eq!(
            explanations[0].relations[0].relation_kind,
            duchy::RelationKind::ImperialState
        );
        assert_eq!(explanations[0].relations[0].overlap, "1050..1100");
    }

    #[test]
    fn serializes_parentage_rank_skip_bridge_tsv_row() {
        let row = ParentageRankSkipRow {
            fact_id: "fact-demo-skip".to_string(),
            child_id: "title-demo-child".to_string(),
            child_name: "Demo Child".to_string(),
        ...
        let candidate = ParentageRankSkipCandidate {
            title_id: "title-demo-kingdom".to_string(),
            title_name: "Demo Kingdom".to_string(),
            title: "Demo Kingdom (title-demo-kingdom)".to_string(),
            rank: duchy::TitleRank::Kingdom,
            exists: "1..20".to_string(),
            overlap_years: 20,
            bridge_fact_id: Some("fact-demo-bridge".to_string()),
        };

        let tsv = parentage_rank_skip_bridge_tsv_row(&row, &candidate);
    ...
    #[test]
    fn parses_parentage_rank_skip_bridge_cluster_tsv_rows() {
        let input = concat!(
            "candidate_parent_id\tcandidate_parent_name\tcurrent_parent_id\tcurrent_parent_name\texpected_parent_rank\tchild_count\thigh\tmedium\tlow\tspan_range\tchild_ids\tchild_names\tskip_fact_ids\tbridge_fact_ids\n",
            "title-kingdom\tDemo Kingdom\ttitle-empire\tDemo Empire\tKingdom\t2\t2\t0\t0\t1..30\ttitle-child-a,title-child-b\tDemo Child A; Demo Child B\tfact-a,fact-b\tfact-bridge\n",
        );

        let rows =
            parentage_rank_skip_bridge_cluster_rows_from_tsv(input).expect("rows should parse");

        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0].child_count, 2);
        assert_eq!(rows[0].high, 2);
        assert_eq!(rows[0].bridge_fact_ids, "fact-bridge");
    }

    #[test]
    fn renders_parentage_rank_skip_bridge_cluster_review_rows_as_not_inferred() {
        let input = concat!(
            "candidate_parent_id\tcandidate_parent_name\tcurrent_parent_id\tcurrent_parent_name\texpected_parent_rank\tchild_count\thigh\tmedium\tlow\tspan_range\tchild_ids\tchild_names\tskip_fact_ids\tbridge_fact_ids\n",
            "title-kingdom\tDemo Kingdom\ttitle-empire\tDemo Empire\tKingdom\t2\t2\t0\t0\t1..30\ttitle-child-a,title-child-b\tDemo Child A; Demo Child B\tfact-a,fact-b\tfact-bridge\n",
        );
        ...
        let output = parentage_rank_skip_bridge_cluster_review_to_tsv(&rows[0]);
        let fields = output.split('\t').collect::<Vec<_>>();

        assert_eq!(fields[0], "pending_review");
        assert_eq!(fields[1], "not_inferred");
        assert_eq!(fields[12], "child_to_candidate_parentage_source");
        assert_eq!(
            fields[13],
            "same-parent bridge overlap is a lead, not evidence"
        );
    }

    #[test]
    fn parses_and_counts_parentage_rank_skip_bridge_cluster_review_rows() {
        let input = concat!(
            "review_status\treview_disposition\tcandidate_parent_id\tcandidate_parent_name\tcurrent_parent_id\tcurrent_parent_name\texpected_parent_rank\tchild_count\thigh\tmedium\tlow\tspan_range\tevidence_needed\treview_note\tchild_ids\tchild_names\tskip_fact_ids\tbridge_fact_ids\n",
        ...
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0].review_status, "pending_review");
        assert_eq!(rows[0].review_disposition, "not_inferred");
